...
DIRNAME=$(dirname $0)
RELEASE_FILE=${DIRNAME}/../files/pas-release-notes.txt
...
echo "# GENETATED BY getPASReleases.sh (`date`)" > $RELEASE_FILE
...
for rel in $LIST; do
  ...
  while [ $i -lt $cnt ]; do
    ...
    cn=$(echo "$nm" | grep -c "Pivotal Application Service")
